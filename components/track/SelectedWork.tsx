        ...
        <StaggerGroup className="mt-12 grid gap-8 md:grid-cols-3">
          {portfolioProjects.map((project) => (
            <StaggerItem key={project.url}>
              ...
              >
                <div className="relative flex aspect-[16/9] w-full items-center justify-center overflow-hidden bg-primary/5">
                  {project.image ? (
                    <Image
                    ...
                    <svg viewBox="0 0 168 108" className="h-12 w-auto text-primary/20">
                      <polygon points="8,100 30,50 52,100" fill="currentColor" />
                      <polygon points="50,100 80,18 112,100" fill="currentColor" />
                      <polygon points="102,100 132,42 160,100" fill="currentColor" />
                    </svg>
                  )}
                </div>
                  ...
                  <p className="mt-2 flex-1 text-sm text-secondary">
                    {project.description ?? (
                      <span className="italic text-secondary/70">
                        Preview unavailable — this project is hosted on a
                        free-tier server that sleeps when idle, so the site
                        may take up to a minute to load on first click.
                      </span>
                    )}
                  </p>
